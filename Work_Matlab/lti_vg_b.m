...
if stops_sv == 1,
    ...
    sv = lti_fourehsv_b(ah_sv, cd_sv, cause_sv, ps, pd, ph, pr, sg,...
        wdh_sv, wdr_sv, wsh_sv, wsr_sv, wfhd_sv, wfrd_sv,...
        wfsh_sv, wfsr_sv, wfj_sv, stops_sv, kix_sv, tau_sv,...
        dp_s_sv);
    act = lti_actuator_b(ah_act, ar_act, bdamp_act, mact_act, mext_act,...
        pr, ph, pd, wfb_act, wfrl_act, wfhl_act, stops_act, sg);
    tee_wfrod    = lti_summer(-1,1,0,0);
    rl = lti_man_n_vm(l_rl, a_rl, vol_rl, n_rl, sg, beta);
    rc = lti_vol_1(vol_rc, beta, sg);
    tee_wfhead = lti_summer(-1,1,0,0);
    hl = lti_man_n_vm(l_hl, a_hl, vol_hl, n_hl, sg, beta);
    hc = lti_vol_1(vol_hc, beta, sg);
    tee_wfr = lti_summer(1,1,1,0);
    tee_pr = lti_splitter(1,1,0,0);
    tee_wfs = lti_summer(1,1,0,0);
