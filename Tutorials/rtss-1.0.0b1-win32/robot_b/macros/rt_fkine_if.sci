...
function [x, y, typ] = rt_fkine_if(job, arg1, arg2)
// File name:       rt_fkine_if.sci
//
// Function:        rt_fkine_if
//
// Description:     handle the user interface of the Scicos block named "rt_fkine_if"
//
    ...
    select job,
        ...
        case "set" then
            ...
            while %T do,
                ...
                [model, graphics, ok] = set_io(model, graphics, list([robot.n, 1], 1) , list([4, 4], 1), [], []);
                if ok then
                    model.opar = list();
                    ...
                    graphics.gr_i(1) = "xstringb(orig(1), orig(2), [""RTSS""; ""FKine""; """ + robot.name + """], sz(1), sz(2), ""fill"");";
                    x.graphics = graphics;
                    x.model = model;
                    break
                end
        ...
        case "define" then
            model = scicos_model();
            model.sim = list("rtss_scs_fkine_cf4", 4);
            model.in = -1;
            model.in2 = 1;
            model.intyp = 1;
            model.out = 4;
            ...
            model.dep_ut = [%T %F];
            exprs = "rt_robot()";
            gr_i = ["xstringb(orig(1), orig(2), [""RTSS""; ""FKine""; ""noname""], sz(1), sz(2), ""fill"");"];
