...
function [x, y, typ] = rt_be_if(job, arg1, arg2)
// File name:       rt_be_if.sci
//
// Function:        rt_be_if
//
// Description:     handle the user interface of the Scicos block named "rt_be_if"
//
// Annotations:     none
//
...
// $LastChangedDate: 2009-08-08 11:29:41 +0200(sab, 08 ago 2009) $
    ...
    select job,
        ...
        case "set" then
            ...
            while %T do
                [ok, step, x0, exprs] = getvalue(..
                    ["Set discrete integral parameters"; "(Backward Euler method)"],..
                    ["Step"; "Initial state"],..
                    list("vec", 1,"vec", -1),..
                    exprs);
                if ~ok then
        ...
        case "define" then
            ...
            step = 1;
            model = scicos_model();
            model.sim = list("rtss_scs_be_cf4",4);
            model.in = 1;
            model.in2 = 1;
            model.intyp = 1;
            model.out = 1;
