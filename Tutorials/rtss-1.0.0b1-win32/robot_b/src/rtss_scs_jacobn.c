...
/**
 * @file  rtss_scs_jacobn.c
 *
 * @brief Computational Function for the Scicos block named "rt_jacobn_if"
 *
...
#include "../includes/rtss.h"       /* Interface to ROBOT and KINEMATICS modules */
 ...
 */ 
static
int rtss_scs_jacobn_init_(scicos_block * block)
{
 ...
 */
static int
rtss_scs_jacobn_inout_(scicos_block * block)
{
    SCSREAL_COP * q, * jnp;
    rtss_robot * rp = (rtss_robot *) GetWorkPtrs(block);

    q = GetRealInPortPtrs(block, 1);
    jnp = GetRealOutPortPtrs(block, 1);
    rtss_kin_jacobn(jnp, rp, q);
    return(RTSS_SUCCESS);
}
 ...
 */
static int
rtss_scs_jacobn_end_(scicos_block * block)
{
    rtss_robot * rp = (rtss_robot *) GetWorkPtrs(block);
 ...
 */
int
rtss_scs_jacobn_cf4(scicos_block * block, int flag)
{
    switch (flag) {
        case OutputUpdate:
            if (rtss_scs_jacobn_inout_(block) == RTSS_FAILURE)
                rtss_err_call();
            break;

        case Initialization:
            if (GetNopar(block) == 0)
                rtss_err_fail(999, "Problems with RTSS JacobN block. Did you forget to specify the robot model to be simulated?");
            if (rtss_scs_jacobn_init_(block) == RTSS_FAILURE)
                rtss_err_call();
            break;

        case Ending:
            if (GetNopar(block) > 0)
                if (rtss_scs_jacobn_end_(block) == RTSS_FAILURE)
                    rtss_err_call();
            break;
