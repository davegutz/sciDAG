...
/**
 * @file  rtss_scs_frne.c
 *
 * @brief Computational Function for the Scicos block named "rt_rne_if"
 *
 ...
 */ 
static
int rtss_scs_frne_init_(scicos_block * block)
{
 ...
 */
static int
rtss_scs_frne_inout_(scicos_block * block)
{
    SCSREAL_COP * q, * qd, * qdd, * tau;
    rtss_robot * rp = (rtss_robot *) GetWorkPtrs(block);

    q = GetRealInPortPtrs(block, 1);
    qd = GetRealInPortPtrs(block, 2);
    qdd = GetRealInPortPtrs(block, 3);
    tau = GetRealOutPortPtrs(block, 1);
    rtss_dyn_frne(tau, rp, GetInPortCols(block,1), q, qd, qdd, NULL); /* execute RNE algorithm */
    return(RTSS_SUCCESS);
}
 ...
 */
static int
rtss_scs_frne_end_(scicos_block * block)
{
    rtss_robot * rp = (rtss_robot *) GetWorkPtrs(block);
 ...
 */
int
rtss_scs_frne_cf4(scicos_block * block, int flag)
{
    switch (flag) {
        case OutputUpdate:
            if (rtss_scs_frne_inout_(block) == RTSS_FAILURE)
                rtss_err_call();
            break;

        case Initialization:
            if (GetNopar(block) == 0)
                rtss_err_fail(999, "Problems with RTSS RNE block. Did you forget to specify the robot model to be simulated?");
            if (rtss_scs_frne_init_(block) == RTSS_FAILURE)
                rtss_err_call();
            break;

        case Ending:
            if (GetNopar(block) > 0)
                if (rtss_scs_frne_end_(block) == RTSS_FAILURE)
                    rtss_err_call();
            break;
